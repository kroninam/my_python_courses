...
#
# Перед вами имеется список кортежей names. Каждый кортеж состоит из двух элементов:
# имени и фамилии. Ваша задача на основании списка names создать новый список new_names,
# где каждый кортеж должен замениться на строку конкатенации имени и фамилии, между которыми
# стоит пробел . Вот пример на других данных:
#
...
new_names = list(map(lambda x: (str(x[0]) + ' ' + str(x[1])), names))
print(new_names)